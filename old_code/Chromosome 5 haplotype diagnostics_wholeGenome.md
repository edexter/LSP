...
This project maps reads to three different references genomes, each containing a different variant forms of a large structural polymorphism (LSP) on chromosome 5. Missingness and read depth are then used to determine which versions of the LSP are present for many different D. magna clones as we have sequence data for.



# Map reads to Haplotype 1 reference

We first have to create the required index files for the reference genome. This can be performed interactively. This needs to be performed separately for each reference genome.
...
#Reference FASTA name
REF=CH_434-inb3-a-1.falcon.polish.mask.fasta
