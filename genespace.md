...
This module performs a genome synteny analysis across a set of reference Daphnia genome assemblies using the GENESPACE software, and then plots the results for chromosome 5 using custom R scripts. This module is divided into several bash scripts which are submitted as batch jobs, and several R scripts which can be run interactively.

## Step 1: format_genomes.sh

This script performs various formatting operations on the input  genome assembly files required for the GENESPACE program. The input genomes need to be stored in the "genomes_original" directory and indexed using a file named "genome_index.txt"
...
## Step 2: repeat_model.sh

The next module will mask repetitive sections of the genome, but first a general repeat model needs to be developed for the species.  Repeat modeling is the very slow process requiring several days with 32 threads. The model is based on the most complete (and thus contains the most repeat content) assembly currently available.
...
## Step 3: annotate_genomes.sh

This scripts performs gene prediction on the (now properly formatted) genome assemblies using AUGUSTUS and INTERPROSCAN. Repetitive sections of the genome are masked using REPEATMASKER before gene prediction is performed. The "genome_index.txt" file is again required. This script completes in a little under 24 hours when submitted as an array job.
...
This script performs synteny analysis using the files generated in the previous steps as input for GENESPACE. This script performs some preliminary setup for the GENESPACE run and then calls an R script to perform the GENESPACE analysis. The contents of the "genespace.r" script are given in the next code block.
...
This R script is called by the previous bash script. The completed run files are all saved to disk and can be reloaded in an interactive session for refinement.
...
## Step 5: Interactive plot development

The "genespace.r" script in the previous step produces a basic riparian plot, which usually requires some adjustment. This script loads the R workspace from the previous script and fine tunes the riparian plot, without the need to re-run the full GENESPACE analysis. First, we need to request the resources for an interactive job on the cluster and start the r session.
...
Next we run the following R code interactively and adjust the plotting parameters as needed.
