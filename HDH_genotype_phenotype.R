...
sampleVariables <- list(
  CH14 = list(
    ...
    LSP_START_POS = 1534355,
    LSP_STOP_POS = 7111581,
    HOMOZYGOTE_RATIO = 1.1,
...
performAnalysis <- function(vars) {
  ...
  # Remove samples with coverage to low to reliably genotype
  df <- df[,colnames(df) != "G389",]
  df <- df[,colnames(df) != "E252",]
  df <- df[,colnames(df) != "D803",]
  df <- df[,colnames(df) != "G255",]
  df <- df[,colnames(df) != "G041",]
  df <- df[,colnames(df) != "D813",]
...
markers <- merge(res, P20, by = "ID")

temp <- markers[markers$phenotype == "RRS" | markers$phenotype == "RRR",]

table(temp$state,temp$phenotype)
